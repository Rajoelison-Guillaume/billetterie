...
<div class="container py-4">
    <h2 class="text-primary fw-bold mb-4">✏️ Modifier une salle</h2>
    ...
    <form action="{{ route('admin.rooms.update', $room->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="venue_id" class="form-select" required>
                @foreach($venues as $venue)
                    <option value="{{ $venue->id }}" {{ $room->venue_id == $venue->id ? 'selected' : '' }}>
                        {{ $venue->name }}
                    </option>
                @endforeach
            </select>
        ...
        <a href="{{ route('admin.rooms.index') }}" class="btn btn-secondary">⬅️ Annuler</a>
    </form>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const niveaux = 10;       // nombre de rangées
    const baseSeats = 30;     // sièges au premier niveau
    const increment = 2;      // incrément par niveau
